...
To upgrade to MUnit 1.0.0, you need to modify the `pom.xml` file.
...
MUnit 1.0.0 requires only the following artifacts:
...
MUnit 1.0.0 requires the following properties in the `pom.xml` file:
...
Note that `${mule.munit.support.version}` should match the Mule ESB version, i.e.:
