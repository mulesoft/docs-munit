...
For this example, we are using properties `${ftp.host}`, `${ftp.port}`, `${ftp.user}` and `${ftp.password}`. +
Remember to add these properties in your `ftp.properties` file in the `src/test/resources` folder:
