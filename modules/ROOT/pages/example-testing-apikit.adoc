...
Create a new Mule Studio project and create a RAML definition file in the `api` folder (for details on how to do this, see the xref:3.x@apikit::apikit-tutorial.adoc[APIkit Tutorial]).
...
[NOTE]
--
The Scaffolder that generates the MUnit test from the RAML definition does not consider if your application uses xref:3.9@mule-runtime::tls-configuration.adoc[TLS configuration]. +
...
You can use APIkit in Mule runtime with or without API Gateway. API Gateway Runtime 1.3 - 2.x has a xref:1.x@api-manager::api-gateway-domain.adoc[domain named api-gateway].
