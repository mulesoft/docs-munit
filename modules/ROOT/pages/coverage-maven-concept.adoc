...
Configure MUnit coverage reports in Maven by adding the munit-maven-plugin to your pom.xml and enabling the coverage-report goal. Coverage reports are generated during the Maven site lifecycle and are not available when running tests from Anypoint Code Builder or Anypoint Studio.
