...
Since MUnit 2.x, the coverage report goal is integrated with the Maven reporting section. MUnit generates Coverage Reports during Maven `site` lifecycle, during the `coverage-report` goal.
...
* HTML report +
The HTML report shows the same information as the console report, but formatted to fit any web browser. It's available in the `${application.path}/target/site/munit/coverage/` path.
...
====================================================================================
----
<1> Warning detailing the which coverage level was not met, and where it happened.
