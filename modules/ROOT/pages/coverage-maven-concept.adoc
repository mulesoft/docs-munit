...
* The SONAR report shows the same information as the XML report, in a SonarQube format. It is available in `${application.path}/target/site/munit/coverage/sonar.xml`.
To pass it to your sonar server you need to add property `<sonar.coverageReportPaths>target/site/munit/coverage/sonar.xml</sonar.coverageReportPaths>` to your pom.xml.
