...
Since MUnit 2.2.0, its optional that you write a description in your test. +
...
MUnit tests will automatically fail if they take more than 2 minutes to complete. Sometimes, that time can be too long or too short. +
In that case, you can use the timeOut attribute, where you can set the desired maximum time -in milliseconds- that the test can run before fail. For example:
