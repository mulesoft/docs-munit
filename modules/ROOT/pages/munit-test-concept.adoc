...
To specify that a suite must run if the Mule version is equal to or newer than a specific version, use the `minMuleVersion` attribute of the configuration:
...
You can find scenarios where you must shut down a test. Whether the test is failing or it has inconvenient side effects, you can ignore a specific test so that you don't manually comment out the code.
...
With this attribute, you can validate that a defined error type in your application is thrown. If you define an `errorType` that does not exist in your application, the test doesn't run.
+
[source,xml,linenums]
----
<munit:test name="MUnit-test-suite"
