...
Each MUnit test suite is, behind the scenes, no different from a Mule configuration file. You can use several of the Mule top level elements such as flow, sub-flow, and scripts.
...
To specify that a suite must run if the Mule version that is equal or newer than a specific version, use the
...
// There's no UI to create this representation in Anypoint Code Builder.
...
In MUnit, you must write a description for your test. The description must be useful and representative of the tested scenario. This description displays in the test console before running the test, and also in the reports. The more representative the description, the easier to read and troubleshoot any failures.

==== Ignore a Test

You can find scenarios where you must shut down a test. Whether the test is failing or it has inconvenient side effects, you can ignore a specific test so that you mustn't manually comment out the code.
...
image::munit-test-concept-de4c9.png["The ignore-test checkbox highlighted in Studio."]
...
MUnit ignores this test if the Mule version is prior to `4.1.4`. Other possible functions are `muleVersionEqualTo` and `muleVersionNewerThan`.
...
If you want to validate that the flow or sub-flow you're testing fails and throws a specific error, you can add the attributes `expectedErrorType`, `expectException`, and `expectedErrorDescription` as follows:
...
With this attribute, you can validate that a defined error type in your application is thrown. If you define an `errorType` that does not exist in your application, the test does not run.
+
[source,xml,linenums]
----
<munit:test name="MUnit-test-suite"
...
With this attribute you can validate that an error is thrown and the error has a description that is equals to or contains the expected error description. If you define that your test expects an error and none is thrown or the error thrown description doesn't match, the test fails immediately.
+
[source,xml,linenums]
----
...
As with other testing frameworks, MUnit provides a set of scopes so you can add processing or actions to run before and after the entire MUnit test or MUnit test suite execute:

* Before and After Suite Scopes.
* Before and After Test Scopes.

The ID field for each component must be unique across all your application. The before and after scopes can't share the same name between them, nor have the same name as any flow in your application.

See xref:before-after-scopes-reference.adoc[] reference for more information.
