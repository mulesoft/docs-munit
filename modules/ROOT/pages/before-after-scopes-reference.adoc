...
Use a Before Suite scope to set up state that every test in the suite shares, such as seeding an in-memory database, starting a server utility, or storing values that the tests read. The example seeds a table in the MUnit database server so that all tests query the same data:
...
<munit:before-suite name="before-suite" description="Seeds the test database for the whole suite">
  <dbserver:execute config-ref="MUnit_DB_Server_Config"
    sql="INSERT INTO jobtitlelookup VALUES ('Developer','10','DEV');" />
</munit:before-suite>

<munit:test name="munit.test" description="Test">
  <!-- Every test in this suite can query the seeded row -->
</munit:test>
----
...
Use an After Suite scope to tear down whatever the Before Suite created, so the next suite run starts from a clean state. The example removes the row that the Before Suite inserted:
...
<munit:after-suite name="after.suite" description="Removes the data created for the suite">
  <dbserver:execute config-ref="MUnit_DB_Server_Config"
    sql="DELETE FROM jobtitlelookup WHERE JOBTITLEID='DEV';" />
</munit:after-suite>
----
