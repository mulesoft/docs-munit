...
TIP: This page and related MUnit pages are focused on XML; however, MUnit is fully compatible with Anypoint Studio's graphical user interface. Within the Studio environment, you can create, design and run MUnit tests just like you would Mule applications. You can even use https://docs.mulesoft.com/anypoint-studio/v/5/studio-visual-debugger[Studio's Visual Debugger] to debug your MUnit tests. For an overview of MUnit in Studio, see xref:using-munit-in-anypoint-studio.adoc[Using MUnit in Anypoint Studio].
...
Each MUnit suite is meant to test a particular set of Mule flows. To do that, you need to tell MUnit where those flows are defined in your application structure. For this we use _imports_, which are in fact https://docs.mulesoft.com/mule-user-guide/v/3.7/adding-and-removing-user-libraries#adding-user-libraries[spring imports].
