...
You can also take advantage of DataWeave functions and mappings to set the  attribute `value` in the `then-return` element to easily mock a more complex POST message. +
As an example, just create a `mockPost.dwl` mapping file under `src\test\resources\sample_data` that creates the payload that you want to return in the POST and then use the `readUrl` DataWeave function to read the mapping file:

[source,xml,linenums]
----
<munit-tools:then-return>
  <munit-tools:payload value="#[readUrl('classpath://sample_data/mockPost.dwl')]" mediaType="application/java" encoding="UTF-8" />
</munit-tools:then-return>
----
