...
The Enable Flow Sources element within the MUnit Test tells MUnit which sources to load and execute for the test. MUnit
does not load any of your flows endpoints by default, it is up to you to configure which endpoints to initialise for
your tests.

For example, assume that in your application you have got a flow with an HTTP listener that returns a specific payload:
...
In order for MUnit to be able to reach the endpoint, you need to enable the {flow_name} in your enable-flow-source
property:
...
----
<1> If the {flow_name} is not listed as an enabled flow source, your MUnit test fails because it cannot initialise the
