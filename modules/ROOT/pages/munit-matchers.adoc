...
MUnit matchers are DataWeave functions that define assertion conditions for any value in an expression, used with the Assert That processor. Include the prefix `MunitTools::` when using them, and choose from core, string, comparable, or iterable and map matcher groups depending on the type of value you want to validate.
