...
image::set-variable-munit-matcher.png[]
...
NOTE: MUnit matchers are _always_ used in combination with the xref:mock-message-processor.adoc[Mocks], xref:spy-message-processor.adoc[Spy], xref:verify-message-processor.adoc[Verify] and xref:assertion-message-processor.adoc[Assert] message processors, but never on their own.
...
image::spy-with-assert-match.png[]
...
image::assert-anyboolean-munit-matchers.png[]
...
image::assert-variable-isboolean-munit-matcher.png[]
...
image::assert-invocation-property-example.png[]
