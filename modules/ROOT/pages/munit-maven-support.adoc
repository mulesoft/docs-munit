...
NOTE: For information on redirecting logs to a file, see xref:logging-in-munit.adoc#redirecting-logs-to-a-file[Logging in MUnit].
...
TIP: If you are using Anypoint Studio, Maven integration is already solved for you. See https://docs.mulesoft.com/anypoint-studio/v/6/using-maven-in-anypoint-studio[Using Maven in Anypoint Studio] for details.
...
The following section covers how to configure xref:munit-coverage-report.adoc[MUnit Coverage].
...
To enable xref:munit-coverage-report.adoc[MUnit Coverage], add the following configuration to the MUnit Plugin:
...
As we've shown before by default, xref:munit-coverage-report.adoc[MUnit Coverage] shows summary report in the console. But that's not the only option.
