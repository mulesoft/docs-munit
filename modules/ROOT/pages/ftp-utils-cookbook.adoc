...
The `contains-files` processor attempts to validate the existence of a file in the FTP server. If the file is not present,
the processor fails, thus causing the test to fail.
