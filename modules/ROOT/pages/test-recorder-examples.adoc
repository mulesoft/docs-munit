= Record an MUnit Test from an Anypoint Exchange Example
...
Record an MUnit test in Anypoint Studio by importing an existing project from Anypoint Exchange and capturing flow processing with the MUnit test recorder. This example uses the Login Form Using the HTTP Connector project.
