...
You cannot return all known error types of a Mule application. You can only return the type of modules used by the current flow. If you try to return an error type that is. out of the scope of the flow you're testing, you get a *MULE:UNKNOWN* error instead of the configured one.

=== Parameters

If you create a mock error, you choose one of the following two parameters to provide the trigger for the error in the *Then-return* tab:

- *TypeId*: It follows a Mule Error type definition. For example, the HTTP:CONNECTIVITY error explained before.
- *Cause*: It expects the instance of the exception with a message for the error. For example, using Java, write the expression:
