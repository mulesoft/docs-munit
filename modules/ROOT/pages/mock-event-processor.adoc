...
You cannot return all known error types of a Mule application. You can only return the type of errors defined in the modules used by the current flow. If you try to return an error type that is out of the scope of the flow you're testing, you get a *MULE:UNKNOWN* error instead of the configured one.
