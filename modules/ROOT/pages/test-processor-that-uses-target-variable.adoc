...
You can test a processor that stores its result in a target variable by asserting the content of the stored variable.
...
The following test mocks the HTTP Request and the resulting target variable from the previos Mule app example, and then asserts that the stored HTTP status code is `200`:
