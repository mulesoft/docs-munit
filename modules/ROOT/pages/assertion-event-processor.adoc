= MUnit Assert That Event Processor
...
The MUnit Assert That event processor validates the state of a Mule event's content during a MUnit test using DataWeave MUnit matchers. Use it to assert conditions on any value in an expression after production code runs.
...
If these assertion fails, the processor throws a `MUNIT-TOOLS:ASSERTION_ERROR`.
