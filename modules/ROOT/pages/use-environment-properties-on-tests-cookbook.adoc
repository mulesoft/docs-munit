...
----
. To load the QA properties during the MUnit tests execution, you need to define an environment variable on the MUnit maven plugin:
+
[source,xml,linenums]
----
 <configuration>
...
----
+
During the MUnit tests execution, MUnit loads the properties from the `qa` properties file and connects to your test environment.
