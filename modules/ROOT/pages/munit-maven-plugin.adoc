...
The following sample references `pirate-pom` file as its parent. It declares the MUnit plugin in a `<plugin>` section without specifying its `<version>` because the MUnit plugin configuration is inherited from the `<pluginManagement>` section in the parent POM file:
...
MUnit has built-in support for SonarQube, with a default setting of `true` for `enableSonarReports`. You mustn't do configuration.
...
You can specify the type of runtime in which the applications you test runs. Accepted values are:
...
Depending on the execution context, the system properties values can vary. When referencing these properties, you must override their value to enforce test reproducibility.
...
|`testOutputDirectory`
|File
|Directory where the test outputs is written. +
