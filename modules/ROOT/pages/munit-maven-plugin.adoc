...
The MUnit Maven Plugin runs MUnit tests as part of a continuous integration environment by integrating with the Maven build lifecycle. These configurations apply only when running MUnit through Maven, not from Anypoint Studio or Anypoint Code Builder.
...
The following configurations only apply when you execute your MUnit tests using the Maven plugin. They don't apply when running tests from Studio. See xref:coverage-maven-concept.adoc[Using Coverage in Maven] for additional information.
...
<plugins>
  <plugin>
    ...
    <configuration>
      ...
      <munit.runtimeversion>4.2.2</munit.runtimeversion>
      ...
    </configuration>
  </plugin>
</plugins>
...
The `munit.runtimeversion` property replaces the deprecated `runtimeVersion` property. The `runtimeVersion` property remains supported. If you use `runtimeVersion`, MUnit displays a warning message that indicates the property is deprecated and that you must migrate to `munit.runtimeversion`.
...
|String
|Deprecated alias for `munit.runtimeversion`. +
This property remains supported for backward compatibility. +
When you use this property, MUnit displays a warning message and instructs you to migrate to `munit.runtimeversion`.
