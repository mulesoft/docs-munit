...
You can declare the MUnit plugin in a parent `pom.xml` file and every child project under this file can choose to reference this definition.

To configure the MUnit Maven plugin in a parent-child POM relationship, you must include the MUnit plugin declaration in the `<pluginManagement>` section of your parent `pom.xml` file.

In the following example, the `pirate-pom` project declares the MUnit Maven plugin in its plugin management section, which defines a global configuration that each child under this parent can choose to reference, override or even ignore.
...
To inherit the MUnit plugin in your child `pom.xml` files, you must reference it in the `<plugin>` section of each child POM file individually.
...
Each child project under a parent `pom.xml` file can choose to ignore the plugin referenced in the  parent's plugin management section and not implement the plugin configuration declared there.
...
When inheriting the plugin from a parent POM file, you can also choose to override the parent's configuration. Overriding the plugin configuration from your parent POM file suppresses the original configuration requiring you to declare all necessary configurations again.
...
After enabling the `<failBuild>`, you must define the coverage level. MUnit Coverage handles three different levels:
...
[CAUTION]
--
Dynamic Ports feature is only available as part of the MUnit Maven Plugin. You can not expect this feature to work when running tests from inside Anypoint Studio.
--
...
A placeholder must parametrize the part of the application trying to make use of a port. For example, to have your Mule application listening for HTTP traffic you must provide the following configuration:
...
In the same way that you instruct MUnit to run one test suite, you can also tell it to run a specific test inside that test suite. You must use the property `munit.test` with the addition of the the special character `#` to append the test name:
...
MUnit allows you to specify the type of runtime in which your applications being tested will run. Accepted values are:
...
MUnit allows you to specify the JVM (or the java executable) in which your applications being tested will run. You must populate the `munit.jvm` parameter with the path to the executable:
...
You can pass additional argument lines to the JVM. You must specify each argument in a separate `argLine`:
...
You may need to define specific system variables so that your MUnit test to run successfully:
