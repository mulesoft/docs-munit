...
|`secure` |(Boolean) Defines the FTP protocol. If you set the `secure` parameter to `true`, the server behaves as an SFTP server and connections must be authenticated. If you set `secure` parameter to `false`, the server behaves as an FTP server and allows either authenticated or anonymous connections. +
