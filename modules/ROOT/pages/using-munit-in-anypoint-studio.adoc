...
Clicking the debug button image:debug-icon.png[] or the play button image:play-icon.png[] on the top right causes the last run or debug to be rerun. This rerun includes all tests that were run on the previous one. You can also select a single test from the previous run to rerun on its own.

== Debugging Tests

You can debug MUnit tests just like Studio applications, using Studio's debugging perspective (for details on the debugging UI, see https://docs.mulesoft.com/anypoint-studio/v/5/studio-visual-debugger[Studio Visual Debugger]).
