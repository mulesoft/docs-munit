...
MUnit 2.2 allows you to write MUnit test suites for your Mule applications that share their resources with a Mule domain without the need for any extra configurations.
...
<mule ...>
  ...
  <munit:validation>
    <munit-tools:assert-equals actual="#[payload]" expected='#["You reached /api"]'/> //<3>
  </munit:validation>
...
. To be able to compare the response against the expected text you must convert the response to plain text.
. Inside the `validation` scope, the `assert-equals` processor asserts that the actual payload (now converted to plain text) is the same as the text you expect.