...
MUnit 2.x enables you to write MUnit test suites for your Mule applications that share their resources with a Mule domain, without performing any additional configuration. To understand how this works, it is helpful to consider a specific scenario populated with actual values, and then use those values in steps that illustrate how to write such a test.
...
Assume an MUnit test to validate that the path being pinged is the expected `/api` path:
