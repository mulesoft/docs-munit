...
NOTE: If you don't create a `log4j2-test.xml` file, Mule looks for a file called `log4j2.xml`. For additional information about the priority in which the log configuration files are selected, see xref:3.7@mule-runtime::logging-in-mule.adoc[Logging in Mule].
